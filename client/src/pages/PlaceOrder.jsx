...
const [country, setCountry] = useState(shippingAddress?.country || '');
  ...
  useEffect(() => {
    ...
    if (success && order?.id) { // Ensure `order` and `order.id` exist
        dispatch({ type: ORDER_RESET });
        dispatch(orderPaymentAction(order.id, paymentResult));
        navigate(`/order/${order.id}`, {});
    }
}, [success, order, paymentResult, navigate]); // Add dependencies
  ...
  const saveShippingAddress = (e) => {
    // Dispatch the saveShippingAddressAction with the required fields
    ...
    console.log("Saving shipping address:", addressData); // Debug the data

    // Now dispatch the action
    dispatch(saveShippingAddressAction({ order: { shippingAddress: addressData } }));
  };
